...
# -*- coding: utf-8 -*-
"""
Created on Wed Jun 10 17:17:54 2020
...
numpoints = 200           # Number of points used in equation solver
...
def mag_eq_a(Mb, lambda_ab, T):
    return Ma_max * brillouin(-mu0 * mua_max * lambda_ab * Mb / (kB*T), Ja)


def mag_eq_b(Ma, lambda_ab, T):
    return Mb_max * brillouin(-mu0 * mub_max * lambda_ab * Ma / (kB*T), Jb)


def equations(mags, lambda_ab, T):
    Ma, Mb = mags
    eq1 = mag_eq_a(Mb, lambda_ab, T) - Ma
    eq2 = mag_eq_b(Ma, lambda_ab, T) - Mb
    return (eq1, eq2)


def get_mag(T_min, T_max, numpoints, lambda_ab, kilo=True):
    #t_start = time.time()
    ...
    for i in range(numpoints):
        ma, mb = fsolve(equations, x0=guess, args=(lambda_ab, Tvec[i]))
        Ma[i] = ma; Mb[i] = mb
        guess = [ma, mb]
...
# Coupling constant
lam_loc = plt.axes([0.125, 0.15, 0.775, 0.03])
lam_ab_init = 500.
lam_ab_max = 800.
lam_ab_min = 200.
...
# Temperature
T_loc = plt.axes([0.125, 0.1, 0.775, 0.03])
T_init = 300.
T_max = 600.
T_min = 1.
...
Ma_curve = mag_eq_a(Ma_scale, lam_ab_init, T_init)
Mb_curve = mag_eq_b(Mb_scale, lam_ab_init, T_init)
...
def update(val):
    ...
    # Update axis 1
    Ma_curve_new = mag_eq_a(Ma_scale, lam_ab_new, T_new)
    Mb_curve_new = mag_eq_b(Mb_scale, lam_ab_new, T_new)
